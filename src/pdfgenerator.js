...
export class PDFGenerator extends Sink {
    ...
    constructor(args) {
        ...
        this.footertext = args.info.coursetitle + ", " +
                args.info.title + ", ";
        if (args.info.ownersdisplaynames) {
            this.footertext += args.info.ownersdisplaynames.join(", ");
            this.footertext += ", "
        }
        this.footertext += new Date(args.info.date).toLocaleDateString('de-DE', {  year: 'numeric', month: 'long', day: 'numeric' });
        ...
        if (args.info.backgroundpdfuse && args.info.backgroundpdf && args.info.backgroundpdf.url) {
            this.backgroundpdf=args.info.backgroundpdf.url;
        }

        this.collection = new Collection((num,dummy)=> (new MemContainer(num,dummy)), { });
    ...
    async initPDF(args)
    {
        if (!this.backgroundpdf) {
            ...
            const pdfbytes = await fetch(this.backgroundpdf).then(res => res.arrayBuffer())
    ...
    addPicture(time, objnum, curclient, x, y, width, height, uuid) {
        ...
        let pictinfo=this.pictures.find((el)=>(el.sha===uuid));
        //console.log("pictinfo",pictinfo);
        if (pictinfo) {
            let addpict=new DrawObjectPicture(objnum);

            addpict.addPicture(x,(y-this.yoffset),
                width,
                height,uuid,pictinfo.url,pictinfo.mimetype); 
    ...
    startPath(time, objnum, curclient, x, y, type, color, w, pressure) {
        ...
        this.workobj[objnum].startPath(x,(y-this.yoffset),
            type,color,w,pressure);
    ...
    {
        let page=this.page;
        let geoscale=this.geoscale;
        for (let i=0;i<this.objects.length;i++)
        {
            let obj=this.objects[i];
            if (obj.type==='glyph') {
                let pathstring=obj.SVGPath();

                let firstpoint=null;
                if (obj.pathpoints && obj.pathpoints.length>0) firstpoint=obj.pathpoints[0];
                ...
                let template = "#000000";
                let strcolor = obj.color.toString(16);
                let mycolor = new tinycolor(template.substring(0, 7 - strcolor.length) + strcolor);
                ...
                if (obj.gtype === 0 && !this.bw) {
                    ...
                    let workcolor = new tinycolor(mycolor.toString());
                ...
                let mc=mycolor.toRgb();
                if (strokewidth ) {
                    let sc=strokecolor.toRgb();
                    page.drawSvgPath(pathstring, {
                        x: this.margins+(sx)/obj.svgscale*geoscale,
                        y: this.margins+this.upageheight+(-(sy)/obj.svgscale)*geoscale,
                ...
                page.drawImage(image, {
                    ...
                    y: this.margins+this.upageheight+(-obj.posy)*geoscale-obj.height*geoscale,
        ...
        page.drawRectangle({x:0,y:0,width:this.pagewidth, height: this.margins, color: rgb(1., 1., 1.) });
        page.drawRectangle({x:0,y:this.pageheight-this.margins,width:this.pagewidth, height: this.margins, color: rgb(1., 1., 1.) });

        page.drawRectangle({x:0,y:0,width:this.margins, height: this.pageheight, color: rgb(1., 1., 1.) });
        page.drawRectangle({x: this.pagewidth-this.margins, y: 0,width: this.margins, height: this.pageheight, color: rgb(1., 1., 1.) });
    ...
    async createPDF() {
        ...
        while (pagepos <= drawarea.glomax) {
            ...
            if (!this.backgroundpdf || pdfpagepos >=pdfpages.length){
                ...
                pagebreak = drawarea.findPagebreak(pagepos + 0.75 * this.scrollheight,
            ...
            this.collection.redrawTo(dispatch, Math.floor(pagepos), Math.ceil(pagebreak));
